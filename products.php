<?php require "libs/signup.php";
    ...
    $tovary = R::Find('products', "type = 'smart'");
?>
<?php
    ...
    if ($category){
    $prods = R::find('products', "`prod_cat` LIKE ? and`type` = 'smart'  ORDER BY `name` ASC", array($category));
    }else {
        $prods = R::Find('products', "type = 'smart'");
    }
	
	if ($color){
    $prods = R::find('products', "`color` LIKE ? and`type` = 'smart'  ORDER BY `name` ASC", array($color));
    }
	

	if ($price==1500){
    $prods = R::find('products', "`price` <= 1500 and`type` = 'smart'  ORDER BY `name` ASC", array($price));
    }
	if ($price==5000){
    $prods = R::find('products', "`price` <= 5000 and`type` = 'smart'  ORDER BY `name` ASC", array($price));
    }
		if ($price==10000){
    $prods = R::find('products', "`price` <= 10000 and`type` = 'smart'  ORDER BY `name` ASC", array($price));
    }
			if ($price==100000){
    $prods = R::find('products', "`price` > 10000 and`type` = 'smart'  ORDER BY `name` ASC", array($price));
    }
	$zol=R::getRow("SELECT count(`color`) as Кол FROM `products` WHERE `color`='Золотой' and `type` = 'smart'");
		$black=R::getRow("SELECT count(`color`) as Кол FROM `products` WHERE `color`='Чёрный' and `type` = 'smart'");
			$red=R::getRow("SELECT count(`color`) as Кол FROM `products` WHERE `color`='Красный' and `type` = 'smart'");
				$grey=R::getRow("SELECT count(`color`) as Кол FROM `products` WHERE `color`='Серый' and `type` = 'smart'");
					$blue=R::getRow("SELECT count(`color`) as Кол FROM `products` WHERE `color`='Синий' and `type` = 'smart'");
		...
		<div class="container">
			<div class="w3ls_mobiles_grids">
				<div class="col-md-4 w3ls_mobiles_grid_left">
              	...
              	<li><a href="products.php?category=Смартфоны">Смартфоны</a></li>
					<li><a href="products.php?category=Ноутбуки">Ноутбуки</a></li>
							  ...
							  <div class="panel panel-default">
								...
								<div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">
								   <div class="panel-body panel_text">
									<ul>
									<li><a href="products.php?category=Прочее">Прочее</a></li>
									<li><a href="products.php?category=Аудио">Аудио</a></li>
					...
					<div class="w3ls_mobiles_grid_left_grid">
						...
						<div class="w3ls_mobiles_grid_left_grid_sub">
							<div class="ecommerce_color">
								<ul>
									<li><a href="products.php?color=Красный"><i></i> Красный(<?php echo $red['Кол'] ?>)</a></li>
									<li><a href="products.php?color=Серый"><i></i> Серый(<?php echo $grey['Кол'] ?>)</a></li>
									<li><a href="products.php?color=Чёрный"><i></i> Чёрный(<?php echo $black['Кол'] ?>)</a></li>
									<li><a href="products.php?color=Золотой"><i></i> Золотой(<?php echo $zol['Кол'] ?>)</a></li>
									<li><a href="products.php?color=Синий"><i></i> Синий(<?php echo $blue['Кол'] ?>)</a></li>
								</ul>
							</div>
						</div>
					</div>
					<div class="w3ls_mobiles_grid_left_grid">
						...
						<div class="w3ls_mobiles_grid_left_grid_sub">
							<div class="ecommerce_color ecommerce_size">
								<ul>
									<li><a href="products.php?price=1500">до 1500 руб</a></li>
									<li><a href="products.php?price=5000">до 5000 руб</a></li>
									<li><a href="products.php?price=10000">до 10000 руб</a></li>
									<li><a href="products.php?price=100000">более 10000 руб </a></li>
								</ul>
							</div>
						</div>
					</div>
				...
				<div class="col-md-8 w3ls_mobiles_grid_right">
					...
					<div class="w3ls_mobiles_grid_right_grid2">
						...
						<div class="w3ls_mobiles_grid_right_grid2_right">
							<select id="country1" name="country1" class="select_item">
								
								  <option value="nameasc">Имени (А - Я)</option> 
				        <option value="namedesc">Имени (Я - А)</option>
							 ...
							 <input type="hidden" name="cat" id="hid" value="<?php echo $category ?>">
							 	  <input type="hidden" name="color" id="hid1" value="<?php echo $color ?>">
							  	  <input type="hidden" name="price" id="hid2" value="<?php echo $price ?>">
						</div>
						<div class="clearfix"> </div>
					</div>
							...
							<div class="agile_ecommerce_tab_left mobiles_grid">
								...
								<div class="simpleCart_shelfItem">
									...
									<form action="#" method="post">
										...
										<button type="submit" id="<?php echo $prod->id ?>" name="cart" class="w3ls-cart">В корзину</button>
									</form>
